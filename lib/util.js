...
function constructAltURLs(urls) {
  ...
  let bbcRedirectUrls = urls.map(url => {
    let index = url.indexOf('bbc.co.uk');
    if (index != -1) {
      return url.substr(0, index + 4) + 'com' + url.substr(index + 9);
    } 
    else {
      index = url.indexOf('bbc.com');
      if (index != -1) {
        return url.substr(0, index + 4) + 'co.uk' + url.substr(index + 7);
      } 
    }

  }).filter(url => url);

  let extendedUrls = urls.concat(bbcRedirectUrls);
  ...
  let indexAlternativeUrls = urls.map(url => {
    ...
    if (index != -1)
      ...
      let slashInsertion = url[url.length - 1] == '/' ? '' : '/';
      return `${url}${slashInsertion}index.html`;
    }
      
  });
  
  extendedUrls = extendedUrls.concat(indexAlternativeUrls);


  let slashAlts = urls.map(url => {
    let slashIsAtEnd = url[url.length - 1] == '/';
    if (slashIsAtEnd)
  ...
  return extendedUrls.concat(slashAlts);
}
