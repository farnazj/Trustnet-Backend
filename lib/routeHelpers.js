  ...
  if (!existingPost) {
    ...
    try {
      ...
      if (articleMetaData.publisher) {
        ...
        .then( async ([articleSource, created]) => {

          if (!articleSource.photoUrl)
            articleSoruce.update({ photoUrl: articleMetaData.logo });

          return Promise.all([articleSource.addInitiatedPost(post), handleBoostPostRelations(articleSource, post)]);
        })
