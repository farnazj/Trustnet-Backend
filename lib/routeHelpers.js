...
async function setBoostTargets(source, boost, targetUsernames, targetLists, post) {
  ...
  if (targets.length) {
    communications.notifyBoostTargetsOfShare(targets, source, post.id);
...
async function importPost(postUrl, source, assessmentObj, authUserIsSharing, targetUsernames, targetLists) {
  ...
  let sanitizedUrl = util.extractHostname(postUrl);
  ...
  if (!existingPost) {
    ...
    try {
      ...
      post = await db.Post.create({
        ...
        description: articleMetaData.description.trim(),
        body: articleMetaData.description.trim(),
        url: sanitizedUrl,
      ...
      */
      ...
      /*
      Regardless of whether or not the post existed before (or the post is newly imported), the assessment
      sent alongside the post by the auth user is associated with the post.
      */
      if (assessmentObj) {
        ...
        if (feed) {
          ...
          */
          if (psl.get(util.extractHostname(feed.rssfeed, true)) == psl.get(util.extractHostname(post.url, true))) {
            ...
            let results = await db.Source.findOrCreateSource(articleMetaData.publisher);
