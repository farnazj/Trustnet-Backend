...
async function setBoostTargets(boost, targetUsernames) {
...
async function handleBoostPostRelations(source, post, targetUsernames) {
  ...
  promises.push(...[
    ...
    setBoostTargets(boost, targetUsernames)
  ]);
...
async function boostPost(source, postId, targetUsernames) {

  let post = await db.Post.findByPk(postId);
  await handleBoostPostRelations(source, post, targetUsernames);
}


async function initiatePost(source, post, headerTargetUsernames) {
  ...
  let boostProms = handleBoostPostRelations(source, post, headerTargetUsernames);

  return Promise.all([assessmentProms, boostProms, postProm]);
}


async function importPost(source, postUrl, assessmentObj, targetUsernames) {
  ...
  if (!existingPost) {
    ...
    try {
      let articleOgp = await ogpHelpers.getOGPArticle(postUrl);
      ...
      handleBoostPostRelations(source, post, targetUsernames);
    }
    catch(err) {
    ...
    handleBoostPostRelations(source, existingPost, targetUsernames);
  }
