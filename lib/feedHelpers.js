...
var headerHelpers = require('./headerHelpers');
        ...
        return Promise.all(articlesInFeed.map(article => {
          ...
          return db.Post.findOne({
            ...
            if (!post) {
              ...
              let universalLink = util.isValidUrl(article.guid) ? article.guid : article.link;

              return Promise.all([ metaScraperHelpers.getArticleMetaData(universalLink), headerHelpers.getPostURLIframeInfo(universalLink) ])
              .then( ([articleMeta, iframeIsAllowed]) => {
                ...
                return db.Post.create({
                  ...
                  (article.pubDate ? article.pubDate : moment.utc()),
                  opinion: articleMeta.opinion,
                  iframeIsAllowed: iframeIsAllowed
                })
                .then(dbPost => {

                    return routeHelpers.initiatePost(source, dbPost, true)
                    .then(x => {
                      let tagProms = [];
                ...
                .catch(err => {
                  logger.error(`Couldn't create post or add it to the initated posts of user. Error: ${err}`);
                })
              })
              .catch(err => {
                logger.error(`Couldn't retrieve the metadata of the article ${universalLink} from feed ${feed.name}. Error: ${JSON.stringify(err)}`)
              })
            }
          })
          .catch(err => {
