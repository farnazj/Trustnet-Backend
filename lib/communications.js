...
async function resolveArbiterSetup(inquirer, arbiter, post) {
  return new Promise((resolve, reject) => {
    if (arbiter.isVerified) {
      ...
      help them by providing your assesment.</p>
      <br>
      <p>-The ${constants.SITE_NAME} team</p>`
      resolve(htmlText);
    }
...
async function notifyArbiters(inquirer, arbiters, post) {

  arbiters.forEach(arbiter => {
    let notifyQuestionMailOptions;
    resolveArbiterSetup(inquirer, arbiter, post)
    .then( htmlText => {
