...
async function getBoostersandCredSources(req) {
  ...
  else if (reqSource == constants.CRED_SOURCES.USERNAMES) //as is, you can check anyone's opinion, doesn't need to be followed or trusted by you
   {
    let sourcePromises = [];
    if (!req.headers.usernames)
      ...
      credSources = (await Promise.all(sourcePromises)).map(source => {return source.id});
    }
   }
...
function buildBoostQuery(req, boostersIds, credSources) {
  ...
  rawSql += ' ORDER BY `Post`.`updatedAt` DESC LIMIT :offset, :limit;'
...
async function getPostBoosts(postIds, req, boostersIds, assessorsIds) {
  ...
  let postBoosts = await db.Post.findAll({
    ...
    group: ['Post.id', 'PostBoosts.id', 'PostAssessments.id', 'PostBoosts->Targets.id', 'PostCustomTitles.id']
  });
