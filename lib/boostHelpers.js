...
function buildBoostQuery(req, boostersIds, credSources) {
  ...
  let reqTags = req.headers.tags ? req.headers.tags.split(constants.STRINGIFIED_ARR_SEP) : undefined;
  ...
  rawSql += '(`PostBoosts`.`SourceId` IN :auth_user_id OR (`PostBoosts`.`SourceId` IN :boosters_ids AND \
