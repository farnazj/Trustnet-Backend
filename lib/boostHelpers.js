...
function buildBoostQuery(req, relations, exploreMode) {
  ...
  ON `PostAssessments`.`id` = `PostAssessments->Arbiters->AssessmentArbiters`.`AssessmentId`';
  */
  ...
  if (trusters.length &&
      [constants.VALIDITY_TYPES.QUESTIONED, constants.VALIDITY_TYPES.ALL].includes(reqValidity) )
    rawSql += ' OR (`PostAssessments`.`postCredibility` = 0 AND `PostAssessments`.`SourceId` IN :trusters)'
...
function anonymizeTrustersQuestions(posts, relations) {
  let unknownTrusters = relations.trusters.filter(sourceId =>
    !relations.followedTrusteds.includes(sourceId));

  posts.forEach(post => {
    let assessments = post.PostAssessments.map(assessment => {
      if (unknownTrusters.includes(assessment.SourceId)) {
