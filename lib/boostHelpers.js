...
function buildBoostQuery(req, relations, exploreMode) {
  ...
  LEFT OUTER JOIN `StandaloneTitles` ON `StandaloneTitles`.`PostId` = `Post`.`id` \
  LEFT OUTER JOIN `CustomTitles` AS `StandaloneCustomTitles` ON `StandaloneCustomTitles`.`standaloneTitleId` = `StandaloneTitles`.`id` AND \
  `StandaloneCustomTitles`.`SourceId` IN :title_contributors ';
  ...
  rawSql += '(`PostBoosts->Targets`.`id` IS NULL OR `PostBoosts->Targets`.`id` IN :auth_user_id)) \
    OR `StandaloneCustomTitles`.`id` IS NOT NULL ';
...
async function buildActivityQuery(req, activityUser, authUserId) {
  ...
  LEFT OUTER JOIN `StandaloneTitles` ON `StandaloneTitles`.`PostId` = `Post`.`id` \
  LEFT OUTER JOIN `CustomTitles` AS `StandaloneCustomTitles` ON `StandaloneCustomTitles`.`standaloneTitleId` = `StandaloneTitles`.`id` AND \
  `StandaloneCustomTitles`.`SourceId` IN :user_id ';
  ...
  rawSql += '(`PostBoosts`.`SourceId` IN :user_id OR \
    `StandaloneCustomTitles`.`id` IS NOT NULL  OR \
...
async function getPostBoosts(postIds, req, relations, exploreMode, activityUser) {
  ...
  */
  ...
  let postBoosts = await db.Post.findAll({
    ...
    group: ['Post.id', 'PostBoosts.id', 'PostAssessments.id', 'PostAssessments->Arbiters.id', 'PostAssessments->Reasons.id',
    'PostBoosts->Targets.id', 'StandaloneTitle.id', 'StandaloneTitle->StandaloneCustomTitles.id',
    'StandaloneTitle->StandaloneCustomTitles->Endorsers.id',
