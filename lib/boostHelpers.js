...
async function buildActivityQuery(req, activityUser, authUserId) {
  ...
  rawSql += ' GROUP BY `Post`.`id` \
  ORDER BY `Post`.`createdAt` DESC LIMIT :offset, :limit;'
    ...
    postWhereClause = {
     ...
     boostsConfig,
     {
      ...
      [Op.or]: [
        {
          /*
          non-question assessments from the initiator of posts and the auth user
          */
          [Op.and]: [ 
           {
        ...
        {  /*non-anonymous questions from those who trust the auth user or 
...
function buildActivityWhereClause(postIds, req, relations, activityUser, authUser) {
  ...
  let questionPoserAnonymity = [null, false];
  if (authUser.id == activityUserId)
    questionPoserAnonymity.push(true);
    ...
    [Op.and] : [
      {
        '$PostBoosts.SourceId$': {
          [Op.or]: {
            [Op.eq]: Sequelize.col('Post.SourceId'),
            [Op.in]: boostersIds.concat([req.user.id, activityUserId])
          }
        }
      },
      { //boosts that are either publicly shared with everyone or with the auth user
        '$PostBoosts->Targets.id$': {
          [Op.or]: {
            [Op.eq]: null,
            [Op.in]: [req.user.id]
  ...
  let postWhereClause = {
    ...
    boostsConfig,
    {
      [Op.or]: [
        {
          /*
          non-question assessments from the initiator of posts, owner of the profile,
          and the auth user
          */
          [Op.and]: [ 
          {
          '$PostAssessments.SourceId$': {
            [Op.or]: {
              [Op.eq]: Sequelize.col('Post.SourceId'),
              [Op.in]: assessorsIds.concat([req.user.id, activityUserId])
            }
          }
          }, {
          '$PostAssessments.postCredibility$': {
...
async function getPostBoosts(postIds, req, relations, exploreMode, activityUser) {
  ...
  if (!activityUser)
    ...
    postWhereClause = buildActivityWhereClause(postIds, req, relations, activityUser, req.user)

  let customTitlePosters = relations.boosters.concat(Sequelize.col('Post.SourceId'));
  if (activityUser)
