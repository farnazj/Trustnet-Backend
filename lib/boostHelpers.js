...
async function getBoostersandCredSources(req) {
  ...
  //crediblity criteria
  let cred_sources;
  if (req.headers.source.toLowerCase() == constants.CRED_SOURCES.TRUSTED)
    cred_sources = auth_user_.Trusteds.map(source => {return source.id});
  else if (req.headers.source.toLowerCase() == constants.CRED_SOURCES.ME)
    cred_sources = [auth_user_.id];
  else if (req.headers.source.toLowerCase() == constants.CRED_SOURCES.USERNAMES) //as is, you can check anyone's opinion, doesn't need to be followed or trusted by you
   {
    let source_promises = [];
...
function buildBoostQuery(req, boosters_ids, cred_sources) {
  ...
  if (req.headers.validity.toLowerCase() == constants.VALIDITY_TYPES.CONFIRMED) {
    min_range = [constants.VALIDITY_CODES.QUESTIONED, constants.VALIDITY_CODES.CONFIRMED];
    max_range = [constants.VALIDITY_CODES.CONFIRMED];
  }
  else if (req.headers.validity.toLowerCase() == constants.VALIDITY_TYPES.REFUTED) {
    min_range = [constants.VALIDITY_CODES.REFUTED];
    max_range = [constants.VALIDITY_CODES.REFUTED, constants.VALIDITY_CODES.QUESTIONED];
  }
  else if (req.headers.validity.toLowerCase() == constants.VALIDITY_TYPES.DEBATED) {
    min_range = [constants.VALIDITY_CODES.REFUTED];
    max_range = [constants.VALIDITY_CODES.CONFIRMED];
  }
  else { //all
    min_range = [constants.VALIDITY_CODES.REFUTED, constants.VALIDITY_CODES.QUESTIONED, constants.VALIDITY_CODES.CONFIRMED];
    max_range = [constants.VALIDITY_CODES.REFUTED, constants.VALIDITY_CODES.QUESTIONED, constants.VALIDITY_CODES.CONFIRMED];
  }
  ...
  if (req.headers.validity.toLowerCase() == constants.VALIDITY_TYPES.QUESTIONED) {
  ...
  if (req.headers.seenstatus.toLowerCase() == constants.SEEN_STATUS.SEEN) {
  ...
  else if (req.headers.seenstatus.toLowerCase() == constants.SEEN_STATUS.NOTSEEN) {
