...
function buildBoostQuery(req, relations, exploreMode) {
  ...
  let reqAccuracy = req.headers.accuracy ? req.headers.accuracy.toLowerCase() : undefined;
  ...
  LEFT OUTER JOIN `StandaloneTitles` AS `PostStandAloneTitles` ON `PostStandAloneTitles`.`PostId` = `Post`.`id` \
  ...
  ON `PostStandAloneTitles`.`id` = `StandaloneTitles->OriginalCustomTitles`.`standaloneTitleId` AND \
  `StandaloneTitles->StandaloneCustomTitles`.`SourceId` IN :title_contributors ';
  ...
  if (reqAccuracy == constants.ACCURACY_TYPES.QUESTIONED) {
  ...
  rawSql += '(`PostBoosts->Targets`.`id` IS NULL OR `PostBoosts->Targets`.`id` IN :auth_user_id)) \
    OR `StandaloneTitles->StandaloneCustomTitles`.`id` IS NOT NULL ';
  ...
  //if ([constants.ACCURACY_TYPES.QUESTIONED, constants.ACCURACY_TYPES.ALL].includes(reqAccuracy) ) {
  if ([constants.ACCURACY_TYPES.ALL].includes(reqAccuracy) ) {
  ...
  if (reqAccuracy == constants.ACCURACY_TYPES.CONFIRMED) {
    rawSql += ' HAVING (max(`PostAssessments`.`postCredibility`) > :max_accuracy_threshold \
      AND min(`PostAssessments`.`postCredibility`) >= :neutral_accuracy_threshold )'
  }
  else if (reqAccuracy == constants.ACCURACY_TYPES.REFUTED) {
    rawSql += ' HAVING (max(`PostAssessments`.`postCredibility`) <= :neutral_accuracy_threshold \
      AND min(`PostAssessments`.`postCredibility`) < :min_accuracy_threshold )'
  }
  else if (reqAccuracy == constants.ACCURACY_TYPES.DEBATED) {
    rawSql += ' HAVING (max(`PostAssessments`.`postCredibility`) > :max_accuracy_threshold \
      AND min(`PostAssessments`.`postCredibility`) < :min_accuracy_threshold )'
  }
  ...
  let replacements = {
    ...
    max_accuracy_threshold: constants.ACCURACY_CODES.QUESTIONED,
    neutral_accuracy_threshold: constants.ACCURACY_CODES.QUESTIONED,
    min_accuracy_threshold: constants.ACCURACY_CODES.QUESTIONED,
    oldest_boost_time: moment().subtract(7, 'days').format()
  }
...
async function buildActivityQuery(req, activityUser, authUserId) {
  ...
  `StandaloneTitles->StandaloneCustomTitles`.`id` = `StandaloneTitles->OriginalCustomTitles`.`CustomTitleId`) \
  ON `PostStandAloneTitles`.`id` = `StandaloneTitles->OriginalCustomTitles`.`standaloneTitleId` AND \
  `StandaloneTitles->StandaloneCustomTitles`.`SourceId` IN :user_id ';
  ...
  rawSql += '(`PostBoosts`.`SourceId` IN :user_id OR \
    `StandaloneTitles->StandaloneCustomTitles`.`id` IS NOT NULL  OR \
...
function buildBoostWhereClause(postIds, req, relations, questionArbiters, exploreMode) {
    ...
    postWhereClause = {
     ...
     boostsConfig,
     {
      ...
      [Op.or]: [
        ...
        {
          /*
          any assessments from the sources the auth user has inquired the post's accuracy from
          */
          '$PostAssessments.SourceId$': {
            [Op.in]: questionArbiters
          }
        },
        {  /*
          ...
          [Op.and]: [
            {
             '$PostAssessments.postCredibility$': {
               [Op.eq]: constants.ACCURACY_CODES.QUESTIONED
             }
           },
           {
             [Op.or]: [
...
function buildActivityWhereClause(postIds, req, relations, questionArbiters, activityUser) {
      ...
      [Op.or]: [
        ...
        },
        {
          ...
          */
          ...
          '$PostAssessments.postCredibility$': {
            [Op.ne]: constants.ACCURACY_CODES.QUESTIONED
          }
          }]
        ...
        {
          /*
          any assessments from the sources from whom the auth user has inquired the post's accuracy
          */
          '$PostAssessments.SourceId$': {
            [Op.in]: questionArbiters
          }
        ...
        */
        ...
        {
          '$PostStandAloneTitles->StandaloneCustomTitles.SourceId$': {
...
async function getPostBoosts(postIds, req, relations, exploreMode, activityUser) {
  ...
  /*
  This code adds the assessments of those from whom the auth user has asked for assessment on a post
  to the set of all of the posts that are being retreived. For example, if the user has asked for 
  source A's assessment on post 10, and posts [10, ..., 20] are being retreived, source A's assessments
  on posts [10, ..., 20] are returned as well.
  This can potentially be changed with a subquery
  */
  let questions = await db.Assessment.findAll({
    where: {
      PostId: {
        [Op.in]: postIds
      },
  ...
  let questionArbiters = [];
  questions.forEach(question => {
    questionArbiters.push(...question.Arbiters.map(source => source.id));
  })


  if (!activityUser) {

    postWhereClause = buildBoostWhereClause(postIds, req, relations, questionArbiters, exploreMode);
    queryAttributes = { 
      include: [
    ...
    postWhereClause = buildActivityWhereClause(postIds, req, relations, questionArbiters, activityUser)
    queryAttributes = {
      include: [
        [Sequelize.fn("MAX", Sequelize.col("PostBoosts.createdAt")), "Most_Recent_Boost_Time"],
  ...
  let customTitleRelations = relations.followedTrusteds;
  if (activityUser)
    customTitleRelations = customTitleRelations.concat(activityUser.id);
  ...
  let postBoosts = await db.Post.findAll({
    ...
    'PostBoosts->Targets.id', 'PostStandAloneTitles.id', 'PostStandAloneTitles->StandaloneCustomTitles.id',
    'PostStandAloneTitles->StandaloneCustomTitles->Endorsers.id',
    'Tags.id', 'Tags->PostTags.createdAt',
    ...
    'PostStandAloneTitles->StandaloneCustomTitles->OriginalCustomTitles.createdAt',
    'PostStandAloneTitles->StandaloneCustomTitles->OriginalCustomTitles.updatedAt'
  ]
  });
...
function anonymizeAnonymousQuestions(posts, authUserId) {
  ...
  posts.forEach(post => {
    let assessments = post.PostAssessments.map(assessment => {
      if (assessment.SourceId != authUserId && assessment.postCredibility == constants.ACCURACY_CODES.QUESTIONED) {
