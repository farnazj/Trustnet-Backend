...
class SocialGraph {
  ...
  async calcTransitiveAssessments(post_id, source_id) {
    ...
    if (source_id) { //for local calculation of transitive assessment starting from a source id
      console.log('************avale if e source_id ', source_id, this.graph.predecessors(source_id))

      marked = this.graph.predecessors(source_id).filter(id => !id in sources_w_assessment_ids);
      console.log('************dakhele source_id ', marked)
    }
    else { //for recalculation of all transitive assessments
      marked = all_sources.filter(src => !src.SourceAssessments.length).map(src => src.id.toString());

      let not_picked_ids = [...marked];
      let initialPick = null;
      console.log("***************transitive values", transitive_values)
      console.log("**********not_picked_ids", not_picked_ids)
      while (not_picked_ids.length && initialPick === null) {
        let random_pick = not_picked_ids[Math.floor(Math.random() * not_picked_ids.length)];
        let succ_assessment_owners = this.graph.successors(random_pick).filter(id => id in sources_w_assessment_ids );

        console.log("***************testing -- random pick:", random_pick, " -- successors: ", this.graph.successors(random_pick))
        console.log('**********in picking', succ_assessment_owners)
      ...
      if (initialPick === null) {
          //assessmnethaye hame ro bardar
        for (const id of Object.keys(sources_no_assessment_id_mapping)) {
          if (sources_no_assessment_id_mapping[id].length) {
            console.log("%%%%%%going to destroy assessment for id ", id)
            sources_no_assessment_id_mapping[id].destroy();
          }
        }
        return;
      }
      ...
      console.log("*************initial pick is ", initialPick)
    }
    ...
    while (marked.length) {
      ...
      console.log('**************succs_w_cred', succs_w_cred)
      ...
      if (succs_w_cred.length) {
        ...
        let trans_cred;
        console.log("*********min_succ_cred", min_succ_cred)
        console.log("*********max_succ_cred", max_succ_cred)
        ...
        console.log('*************avg_cred', trans_cred)
    ...
    for (const [id, val] of Object.entries(transitive_values)) {
      if (val != -2)
        ...
        console.log('transitive value for post', post_id, 'for node', id, ' is ', val)
    }
